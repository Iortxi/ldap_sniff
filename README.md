...
2. It'll look for one traffic capture binary that is installed in the remote server. `snoop`, `tcpdump` supported. At [paquetes.py](#paquetespy) you'll find the command execution templates for those binaries, add more if you need to.
3. It'll start the traffic capture in `/tmp/NAME_temp` file at the remote server.
